...
app.get('*', async (c) => {
	const saver = new DurableObjectSaver(
		...
		() => c.env.LANGCHAIN_THREADS.get(c.env.LANGCHAIN_THREADS.idFromName('agent1')) as any
	);
	...
	const result = await graph.invoke({ messages: [] }, { configurable: { thread_id: threadId } });
...
export { DurableGraphObject, DurableThreadObject } from 'checkpoint-durable-object';
